...
module.exports = class ClusterManager {
  ...
  start () {
    ...
    this.startingPromise = co(function * () {
      ...
      var presetWorkers = 0
      var explicitDeclareJobInstances = []
      if (exists(self.app.queueOptions)) {
        Object.keys(self.app.queueOptions).forEach(function(queueName,index) {
          var queueOption = self.app.queueOptions[queueName]
          if (exists(queueOption.numberInstances)) {
            times(queueOption.numberInstances, function () {
              queueNames.push(queueName)
              presetWorkers += 1
            })
            explicitDeclareJobInstances.push(queueName)
          }
        })
      }

      if (exists(COWORKERS_WORKERS_PER_QUEUE) || numCpus > numQueues + presetWorkers - explicitDeclareJobInstances.length) {
        ...
        times(workersPerQueue, function () {
          queueNames = queueNames.concat(
            self.app.queueNames.filter(function(queue) {
              return explicitDeclareJobInstances.indexOf(queue) < 0
            })
          )
        })
      } else {
