...
function Application (options) {
  ...
  Object.defineProperty(this, 'queueOptions', {
    get() {
      var result = {}
      var self = this
      Object.keys(this.queueMiddlewares).forEach(function(queueName,index) {
        // key: the name of the object key
        result[queueName] = self.queueMiddlewares[queueName].jobOpts
      })
      return result
    }
  })
...
Object.assign(Application.prototype, {
   ...
   * @param  {Object} [jobOpts] job options
   * @param  {Number} [jobOpts].numberInstances number job instances
   ...
   * @return {Application} self
   */
  queue (queueName, queueOpts, consumeOpts, jobOpts /*, ...middlewares */) {
    let middlewares
    let args

    if (this.schema) {
      // queueOpts should not be passed if this.schema exists
      assert(!isObject(arguments[3]), 'app.queue() cannot use "queueOpts" when using a schema')
      ...
      jobOpts = args.jobOpts || {}
      middlewares = args.middlewares
      debug('queue %s, %o, %o, %o, %s',
        queueName, queueOpts, consumeOpts, jobOpts,
    ...
    this.queueMiddlewares[queueName].jobOpts = jobOpts
